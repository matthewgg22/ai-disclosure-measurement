...
# `claims_ledger.csv` — chain of custody

Every figure quoted anywhere in this repository or its private write-up, with the file and exact JSON
path that produced it. **No row was typed.** The ledger is generated by walking the result files and
extracting values programmatically, then re-derived and diffed on demand.

| column | meaning |
|---|---|
| `section` | which finding the figure belongs to |
| `claim` | the quantity, in words |
| `value` | the number as quoted |
| `unit` | AUC, %, pp, $B, OR, p, count |
| `source_file` | the result file it came from |
| `json_path` | the exact key inside that file — e.g. `by_pillar_count[pillars=2].median_fwd` |
| `module` | the script that produced the source file |

**Why it exists.** A written summary is where a number drifts from its source: a transcription slip,
a stale figure kept after a re-run, or a plausible value that was never computed. Two claims in this
project were found wrong within a day of being written, both because they read fluently enough to
pass as established. A ledger that is *written* would share that failure mode; one *derived from the
outputs and checked back against them* does not.

**How to use it.** Pick any number in the write-up, find its row, open the named file at the named
path. The private workbook (`AUDIT_TRAIL.xlsx`) additionally carries a RAW COUNTS sheet with the
numerator and denominator behind every rate — so any percentage can be recomputed by hand in the
spreadsheet — and a PROVENANCE sheet with each source file's size, modification time and SHA-256, so
a file changing under a published figure is detectable rather than silent.

**What it does not do.** It verifies that quoted numbers match computed numbers and that the inputs
have not moved. It does not verify that the pipeline is correct — that is methodology review, and a
separate exercise.
