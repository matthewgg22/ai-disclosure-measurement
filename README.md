...
Across 14,282 issuer-vintages (2022–2024) — **every 10-K filer, not a small-cap subset**; median total assets $687M, 15% above $10B — issuers with **no prior listing trouble** and three warning signs in the 10-K went on to file an 8-K Item 4.02 — *previously issued financial statements should no longer be relied upon* — at **12.2%**, against **1.6%** for issuers with none. Monotone, **7.5×**, size-adjusted AUC **0.606** (0.575–0.639).

**Size does real work here and is reported, not waved away.** The flagged firms are small (median total assets $17.9M at two signs, against $2.1B at zero), and small firms fail more, so part of the raw gradient is size: **raw AUC 0.689 falls to 0.605 when computed within size terciles — roughly 44% of the raw discrimination above chance is size.** The 0.605 is what survives that control and is the figure quoted throughout. One detector is sharper than the whole screen: a material asset booked without arm's-length consideration flagged **29 companies in three years, of which 17.2% later retracted** (4.17× lift, Fisher exact p = 0.006) — a precision AUC structurally cannot see, since a flag firing on 0.2% of a population is pinned near 0.5 whatever its precision.
...
**Which signals actually carry it.** A logistic model — sixteen candidate signals entered separately, size continuous, **errors clustered by issuer** (6,192 clusters across 14,282 rows) — gives a deflationary answer. The signals that predict *every* outcome tested are **going-concern doubt** (OR 1.72–2.10) and **material weakness** (1.36–1.95), both p<0.001: things the auditor already wrote down and filed. Dilution markers (share explosion, reverse split, at-the-market facility) predict distress and listing trouble but are flat against restatement — the right pattern, since a reverse split responds to a listing problem rather than symptomising bad accounting. Out of sample the model beats the count score 0.764 vs 0.728 (non-reliance), 0.855 vs 0.751 (any revelation), 0.777 vs 0.714 (price collapse). **The screen is not finding hidden evidence; it is reading existing mandatory disclosure at population scale.**

**Two things a wider signal set revealed.** Adding seven validated registry surfaces showed the result was partly **sensitive to the exact phrase string**: `"identified a material weakness"` outperforms the phrasing originally used on every outcome (1.95 vs 1.41 for restatement), which is a measurement-error problem invisible until two phrasings run side by side. And a signal that *should* work does not — 43 issuer-vintages disclosed receiving an **SEC Wells notice**, the staff's advance warning that enforcement is recommended, and **none** subsequently filed a non-reliance 8-K.

**The screen is not just the audit opinion.** Twelve constructs computed from the financial statements alone — no auditor language, no disclosure text — were tested with going-concern and material weakness already in the model, and **ten of twenty-four construct-outcome tests survive**. For price collapse, **operating cash flow below zero (OR 2.71) outweighs both auditor signals** (1.60, 1.43), is computable for 98% of filers, and is a number the issuer reported rather than a sentence written about it; asset opacity is computable for 100% and independently significant at 1.72. The discrimination gain is small (+0.008 to +0.012 AUC) — the balance sheet changes what the screen is made of more than how well it ranks. Beneish's canonical receivables index is null on both outcomes.

**Measuring the harm raises the question of what recovers it, and the answer is structural.** Every monetary remedy in the securities laws is indexed to the **wrongdoer's gain**, never to investor loss: disgorgement is capped at net profits (*Liu v. SEC*, 591 U.S. 71 (2020)), and all three civil-penalty tiers under 15 U.S.C. §78u(d)(3) are capped at the greater of a fixed per-violation figure or *"the gross amount of pecuniary gain to such defendant."* In this population **measurable gain is 13.5% of investor loss** — so perfect enforcement reaches about one-seventh of the harm, by construction rather than by failure. Meanwhile **0.4–2.2%** of issuers with a dated revelation ever appear as an SEC defendant, the **median district-court case orders $0.70M**, and **57.9% fall below the $1,000,000 covered-action floor** that would pay a whistleblower. A longer limitations period was tested and would not have helped. See [docs/POLICY.md](docs/POLICY.md).

**Statistical strength and early warning are in tension.** Measuring each issuer's stock return over the twelve months *ending* at the 10-K filing date separates signals that warn from signals that confirm. Going-concern carriers have **already lost 37%** by the time the filing appears (population median −4.1%); reverse splits, pre-funded warrants and share explosions all sit past −50% and cannot warn anyone. **Three signals are flat going in — asset opacity (−3.3%), negative equity (−2.9%) and thin equity (−4.6%) — and all three are balance-sheet constructs that survive the multivariate model.** They are the only signals here that are both statistically significant with the auditor admissions controlled and genuinely ex ante. Ranked by odds ratio this screen leads with a signal the market has already acted on; ranked by when the information arrives it leads with asset opacity. A regulator wants the second ordering.

**And a premise the project was built on turns out to be backwards — which resolves into a better result.** Reported as nested specifications, going-concern doubt loses more than half its association to controls (6.88pp → 3.05pp) while material weakness barely moves (5.53 → 4.52). Interacting each signal with size then shows the auditor-admission signals discriminate better among *larger* issuers. Estimating separately within size terciles resolves why: **there are two screens here, not one.** A material-weakness disclosure raises the odds of a later restatement **tenfold at an $8.4B company** (OR 10.02) and does nothing measurable at an $18M one — while the dilution apparatus (share explosion, reverse split, pre-funded warrants) predicts *price collapse* among nano-caps and nothing among large issuers. The mechanism is base rates: a material weakness is ambient at nano scale and sharply abnormal at real scale; continuous dilution is the reverse. **Checking two claims that section made then corrected one of them** — the revelation-day loss does *not* concentrate at the small end (89% is mid and large, because loss is CAR × float and float scales with size), and the top-tercile AUC of 0.795 is in-sample only, with too few training events to validate out of sample. Omitted-variable bias is signed rather than waved at, and four of five unobservables push toward **overstatement**. All in [`docs/REVELATION.md`](docs/REVELATION.md), which now also opens with the descriptive table the project should have had first — a zero-sign issuer is a $2.1B company earning $34.6M; a three-sign issuer is an $11.8M company losing money 92% of the time.
...
- Issuers with three warning signs and no prior listing trouble later declared their own financials unreliable at **12.2%** vs **1.6%** for issuers with none — monotone, 7.5×, size-adjusted AUC **0.606**. Size accounts for ~44% of the raw gradient (raw 0.689 → 0.605 within size terciles); the size-controlled figure is the one quoted.
...
- Of nine signals in a **clustered logistic model**, only **going-concern doubt** and **material weakness** predict every outcome (p<0.001) — the auditor's own written admissions. Dilution markers predict distress but not restatement. Out-of-sample AUC 0.753–0.852 vs 0.698–0.751 for the count score.
