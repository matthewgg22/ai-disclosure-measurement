...
**Which signals actually carry it.** A logistic model — each surface entered separately, size continuous, **errors clustered by issuer** (6,192 clusters across 14,282 rows) — shows the answer is deflationary. Of nine candidate signals, the two that predict *every* outcome tested are **going-concern doubt** (OR 1.73–2.19) and **material weakness** (OR 1.33–2.00), both p<0.001: things the auditor already wrote down and filed. The dilution markers (share explosion, reverse split) predict distress and listing trouble but are flat against restatement — the right pattern, since a reverse split responds to a listing problem rather than symptomising bad accounting. Out of sample the model beats the count score 0.753 vs 0.723 (non-reliance), 0.852 vs 0.751 (any revelation), 0.776 vs 0.698 (price collapse). **The screen is not finding hidden evidence; it is reading existing mandatory disclosure at population scale.**
