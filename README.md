...
**And a premise the project was built on turns out to be backwards.** Reported as nested specifications, going-concern doubt loses more than half its association to controls (6.88pp bivariate → 3.05pp fully specified) while material weakness barely moves (5.53 → 4.52) — they are not equally confounded, and only the nested view shows it. Interacting each signal with firm size then shows the auditor-admission signals **discriminate better among *larger* issuers, not smaller** (going-concern × size, p=0.0001): among nano-caps a going-concern paragraph is close to ambient, while for a company with a real balance sheet it is rare and sharply informative. The screen's marginal value is highest where examination attention already is. Omitted-variable bias is signed rather than waved at, and four of five unobservables push toward **overstatement**. All in [`docs/REVELATION.md`](docs/REVELATION.md).
